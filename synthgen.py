...
def get_text_placement_mask(xyz,mask,plane,pad=2,viz=False):
    ...
    """
    contour,hier,_ = cv2.findContours(mask.copy().astype('uint8'),
                                    mode=cv2.RETR_CCOMP,
...
def save_label_txt(filename, lineBB, txt, letterBB):

    nlines = len(txt)
    with codecs.open(filename, 'w',encoding="utf-8") as dest:
        for i in range(nlines):
            dest.write(str(int(lineBB[0,0,i])) + " " + str(int(lineBB[1,0,i])) + " " + str(int(lineBB[0,2,i])) + " " + str(int(lineBB[1,2,i])) + " " + '"' + txt[i] + '"' + "\n")
            dest.write(str(letterBB[i]) + "\n")
...
class RendererV3(object):
    ...
    def __init__(self, data_dir, max_time=None, text_number=1, font_color_user = None, font_size_user = 18, text_content_user = "sample") :
