...
def sample_weighted(p_dict):
    ps = p_dict.keys()
    #fit py3: np.random.choice(a) a must be 1-dimensional or an integer; type(ps) is dict_key
    ps = list(ps)
...
class RenderFont(object):
    ...
    def __init__(self, data_dir, font_size_user, text_content_user):
        ...
        self.p_flat = 0.10
        self.font_size_user = font_size_user
        # curved baseline:
        self.p_curved = 1.0
        self.baselinestate = BaselineState()

        # text-source : gets text:
        from pathlib import Path
        self.text_source = None
        self.text = None
        text_path = Path(text_content_user)
        if text_path.exists():
            self.text_source = TextSource(min_nchar=self.min_nchar,
                                     fn=text_content_user)
    ...
    def render_multiline(self,font,text):
        ...
        for l in lines:
            ...
            for idx, ch in enumerate(l): # render each character
    ...
    def render_sample(self,font,mask):
        ...
        while i < self.max_shrink_trials :#and max_font_h > self.min_font_h:
            ...
            if self.font_size_user != -1:
                f_h = self.font_size_user
    ...
    def get_lines(self, nline, nword, nchar_max, f=0.35, niter=100):
        def h_lines(niter=100):
            ...
            while not np.all(self.is_good(lines,f)) and iter < niter:
                iter += 1
                line_start = np.random.choice(len(self.txt)-nline+1)
